...
const char* ModelException::what() const noexcept
{
	...
	oss << GenException::what() << std::endl
		<< "[Note] " << GetNote();
	whatBuffer = oss.str();
...
// Mesh
Mesh::Mesh(GenGraphics& gfx, std::vector<std::unique_ptr<Bind::Bindable>> bindPtrs)
{
...
void Mesh::Draw(GenGraphics& gfx, DirectX::FXMMATRIX accumulatedTransform) const noxnd
{
...
void Node::Draw(GenGraphics& gfx, DirectX::FXMMATRIX accumulatedTransform) const noxnd
{
	const auto built =
	...
	void Show(const char* windowName, const Node& root) noexcept
	{
		...
		if (ImGui::Begin(windowName))
		{
			...
			if (pSelectedNode != nullptr)
			{
				...
				ImGui::Text("Position");
				ImGui::SliderFloat("X", &transform.x, -20.0f, 200.0f);
				ImGui::SliderFloat("Y", &transform.y, -20.0f, 200.0f);
				ImGui::SliderFloat("Z", &transform.z, -20.0f, 200.0f);
			}
		}
		ImGui::End();
	}
...
Model::Model(GenGraphics& gfx, const std::string fileName)
	:
...
void Model::Draw(GenGraphics& gfx) const noxnd
{
...
std::unique_ptr<Mesh> Model::ParseMesh(GenGraphics& gfx, const aiMesh& mesh)
{
	namespace dx = DirectX;
